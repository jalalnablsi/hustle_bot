...
const startGame = useCallback(async () => {
  ...
  const handleDropBlock = useCallback(() => {
    ...
    if (isPerfect) {
      ...
      if (newConsecutive >= 3 && newConsecutive % 3 === 0) {
        ...
        toast({
          description: <span className="flex items-center"><Gem className="h-4 w-4 mr-1 text-sky-400" /> 3x Perfect! +{gainedDiamonds.toFixed(2)}💎</span>,
          duration: 1500
        });
      }
    } else {
  ...
  const handleSpendDiamonds = async () => {
    ...
    if ((currentUser.diamond_points ?? 0) < DIAMONDS_TO_CONTINUE) {
      toast({ title: "Not Enough Diamonds", variant: "destructive" });
      return;
    }
    setIsApiLoading(true);
    try {
        ...
        <div className="flex flex-col items-center justify-center text-center p-4 space-y-4 max-w-sm w-full animate-in fade-in-50">
          ...
          <div className="w-full space-y-3 pt-4">
            <Button
              ...
              size="lg"
              className="w-full h-12 text-md font-bold bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-white rounded-full backdrop-blur-sm px-6 py-3 shadow-lg hover:shadow-xl transition-all"
            >
              {isApiLoading ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button
                ...
                variant="outline"
                className="h-11 border-sky-500/80 text-sky-400 hover:bg-sky-500/10 hover:text-sky-300 rounded-full backdrop-blur-sm"
              >
                {isAdInProgress ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ) : (
              ...
              <Button
                ...
                variant="secondary"
                className="h-11 rounded-full backdrop-blur-sm"
              >
        ...
        <div className="w-full px-2 sm:px-4 py-2 bg-slate-900/90 backdrop-blur-sm shadow-md border-b border-primary/30 z-20 flex flex-wrap justify-between items-center gap-2">
          <div className="flex items-center space-x-1">
            {Array.from({ length: MAX_POOLED_HEARTS }).map((_, i) => (
              <Heart
                key={`life-${i}`}
                className={cn("h-6 w-6 transition-all", i < hearts ? "text-red-500 fill-red-500" : "text-slate-600 fill-slate-700")}
              />
            ))}
          </div>
          ...
          <p className="text-sm font-bold flex items-center gap-1.5">
            <Award className="h-5 w-5 text-yellow-400" />{highScore}
          </p>
        </div>
