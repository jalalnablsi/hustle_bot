...
export default function StakeBuilderGamePage() {
  ...
  const [isGameApiLoading, setIsGameApiLoading] = useState(false); // For user-initiated API calls (spin, claim, etc.)
  const [isInitialDataLoading, setIsInitialDataLoading] = useState(true); // For initial page data load
  ...
  const parseNextRegenTimeFromUser = useCallback((user: typeof currentUser | null) => {
    if (!user || !user.game_hearts) return null;
    const heartsData = user.game_hearts[GAME_TYPE_IDENTIFIER];
    if (typeof heartsData === 'object' && heartsData !== null && typeof heartsData.nextRegen === 'string') return heartsData.nextRegen;
    // if backend sends user.last_heart_replenished, it could be used to calculate next regen
  ...
  const updateHeartStateFromApi = useCallback((apiData: any, source: string) => {
    ...
    if (apiData.success) {
      ...
      newHeartsCount = parseHeartCountFromUser(currentUser); // Keep current from context
  ...
  const fetchUserGameData = useCallback(async (userIdForFetch: string) => {
    ...
    try {
      ...
      const currentHearts = heartsApiData.success && heartsApiData.hearts?.[GAME_TYPE_IDENTIFIER] !== undefined
        ? heartsApiData.hearts[GAME_TYPE_IDENTIFIER]
        : parseHeartCountFromUser(currentUser);
  ...
  useEffect(() => {
    ...
    if (currentUser?.id && gameState === 'loading_user_data') {
      ...
      setIsInitialDataLoading(false);
      setGameState('idle');
      toast({ title: "User Not Logged In", description: "Please launch via Telegram.", variant: "destructive" });
    }
  }, [currentUser, contextLoadingUser, fetchUserGameData, gameState, toast, parseHeartCountFromUser, parseNextRegenTimeFromUser]);
  ...
  const checkBackendReplenish = useCallback(async (isManualCollectAttempt = false) => {
    ...
    try {
      ...
      if (isManualCollectAttempt && heartsWereUpdatedByApiCall && data.success) {
        const newHearts = (data.hearts && data.hearts[GAME_TYPE_IDENTIFIER] !== undefined) ? data.hearts[GAME_TYPE_IDENTIFIER] : previousHearts;
        if (newHearts > previousHearts) toast({ description: <span className="flex items-center"><Heart className="h-4 w-4 mr-1 text-red-400 fill-red-400" /> Heart(s) collected! New total: {newHearts}</span> });
      }
  ...
  const startGameAttempt = useCallback(async () => {
    if (!currentUser?.id) { toast({ title: "Login Required", variant: "destructive" }); return; }
    if (pooledHearts <= 0 || isGameApiLoading || ['playing', 'ad_viewing'].includes(gameState) || gameAreaWidth <= 0) {
      if (pooledHearts <= 0 && gameState !== 'playing') toast({ title: "No Hearts Left!" });
      else if (gameAreaWidth <= 0 && pooledHearts > 0) toast({ title: "Game Area Not Ready" });
      else if (isGameApiLoading) toast({ title: "Busy" }); return;
    }
    setIsGameApiLoading(true);
    try {
    ...
    } catch (error) { toast({ title: 'Network Error', variant: 'destructive' }); } 
    finally { setIsGameApiLoading(false); }
  ...
  const processAttemptOver = useCallback(async () => {
    ...
    if (currentUser?.id && (finalScore > 0 || currentAttemptGold > 0 || currentAttemptDiamonds > 0)) {
      ...
      try {
        ...
        } else { toast({ title: "Score Submission Issue", description: scoreData.error, variant: "destructive" }); }
  ...
  const continueCurrentAttempt = useCallback(() => {
    if (stackedBlocks.length > 0 && gameAreaWidth > 0) {
      ...
      setGameState('playing');
        ...
        toast({ title: "Game Area Error", variant: "destructive" }); setGameState('idle');
    } else { setGameInitializationPending(true); }
  ...
  const handleDropBlock = useCallback(() => {
    ...
    const overlapStart = Math.max(currentBlock.x, topStackBlock.x), overlapEnd = Math.min(currentBlock.x + currentBlock.width, topStackBlock.x + topStackBlock.width);
    const overlapWidth = Math.max(0, overlapEnd - overlapStart);

    if (overlapWidth >= MIN_BLOCK_WIDTH_INTERNAL * 0.7) {
      newBlockX = overlapStart; newBlockWidth = overlapWidth;
      const xDiff = Math.abs(currentBlock.x - topStackBlock.x), widthDiff = Math.abs(currentBlock.width - topStackBlock.width);
      if (xDiff < PERFECT_DROP_THRESHOLD_INTERNAL && widthDiff < PERFECT_DROP_THRESHOLD_INTERNAL + 2) {
        isPerfectDrop = true; newBlockX = topStackBlock.x; newBlockWidth = topStackBlock.width;
      ...
      } else { gainedGoldThisDrop = GOLD_FOR_SUCCESSFUL_NON_PERFECT_DROP_INTERNAL; setConsecutivePerfectDrops(0); }
      ...
      if (visualNewBlockTopY < GAME_AREA_HEIGHT_MIN_INTERNAL / 2.3 && stackedBlocks.length +1 > 5) setStackVisualOffsetY(prevOffset => prevOffset + INITIAL_BLOCK_HEIGHT_INTERNAL);
      if (gameAreaWidth > 0) { spawnNewBlock(newBlockWidth, newBlockY - stackVisualOffsetY); setGameState('playing'); }
      else processAttemptOver();
    } else processAttemptOver();
  ...
  const gameLoop = useCallback(() => {
    ...
    setCurrentBlock(prev => {
      if (!prev) return null;
      let newX = prev.x + prev.direction * prev.speed, newDirection = prev.direction;
  ...
  const handleAdsgramRewardForHeart = useCallback(() => {
    toast({ title: "Ad Watched!", description: <span className="flex items-center"><Heart className="h-4 w-4 mr-1 text-red-400 fill-red-400" /> Heart reward is processing. Refreshing data...</span>, icon: <Tv className="text-primary" /> });
    setTimeout(() => { fetchUserFromContext(); }, 2000); // Refresh user data to get new heart count
    setIsAdInProgress(false);
  }, [toast, fetchUserFromContext]);

  const handleAdsgramErrorForHeart = useCallback(() => { setIsAdInProgress(false); }, []);
  const handleAdsgramCloseForHeart = useCallback(() => { setIsAdInProgress(false); }, []);
  ...
  const handleWatchAdForHeartClick = async () => {
    if (!currentUser || isAdInProgress || isGameApiLoading) {
      toast({ title: "Cannot Watch Ad", description: isAdInProgress ? "Ad already in progress." : "System busy.", variant: "default" });
      return;
    }
  ...
  const handleSpendDiamondsToContinue = useCallback(async () => {
    ...
    if (diamondContinuesUsedThisAttempt >= MAX_DIAMOND_CONTINUES_PER_ATTEMPT_INTERNAL) {
      toast({ title: "Continue Limit Reached", variant: "default" }); return;
    }
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') { window.addEventListener('resize', handleResize); handleResize(); }
  ...
  useEffect(() => {
    ...
    if (pooledHearts < MAX_POOLED_HEARTS && nextHeartRegenTime && nextHeartRegenTime > Date.now()) {
      const updateTimerDisplay = () => {
        const now = Date.now();
        if (now >= nextHeartRegenTime) { setTimeToNextHeartDisplay("Collect Heart"); if (intervalId) clearInterval(intervalId); }
        else {
          const remainingMs = nextHeartRegenTime - now;
          const hours = Math.floor(remainingMs / (36e5)); const minutes = Math.floor((remainingMs % (36e5)) / (6e4)); const seconds = Math.floor((remainingMs % (6e4)) / 1000);
    ...
    } else if (pooledHearts < MAX_POOLED_HEARTS && (!nextHeartRegenTime || nextHeartRegenTime <= Date.now())) setTimeToNextHeartDisplay("Collect Heart");
    else if (pooledHearts >= MAX_POOLED_HEARTS) setTimeToNextHeartDisplay("Hearts Full!");
    else setTimeToNextHeartDisplay("Loading...");
    return () => clearInterval(intervalId);
  }, [pooledHearts, nextHeartRegenTime]);

  useEffect(() => {
    if (gameInitializationPending && gameAreaWidth > 0 && gameAreaRef.current) { initializeNewGameAttempt(); setGameInitializationPending(false); }
  }, [gameInitializationPending, gameAreaWidth, initializeNewGameAttempt]);

  useEffect(() => {
    if (gameState === 'playing' && currentBlock && gameAreaWidth > 0 && currentBlock.speed > 0 && !gameLoopRef.current) gameLoopRef.current = requestAnimationFrame(gameLoop);
    else if ((gameState !== 'playing' || !currentBlock || (currentBlock && currentBlock.speed <=0)) && gameLoopRef.current) { cancelAnimationFrame(gameLoopRef.current); gameLoopRef.current = null; }
  ...
  const playButtonDisabled = pooledHearts <= 0 || isGameApiLoading || ['playing', 'ad_viewing'].includes(gameState) || gameAreaWidth <= 0 || gameInitializationPending;
        ...
        <div className="flex-grow w-full flex flex-col items-center justify-center overflow-hidden p-2 relative">
          {(gameState === 'playing' || gameState === 'dropping') && gameAreaWidth > 0 && stackedBlocks.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center text-center p-4 space-y-4 max-w-md w-full">
              ...
              {['idle', 'waiting_for_hearts', 'gameover_attempt'].includes(gameState) && <p className="text-xs text-muted-foreground mt-3">Tap screen or press Space to drop blocks. Perfect drops earn more!</p> }
            </div>
          )}
        </div>
      </div>
