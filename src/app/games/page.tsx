...
const PERFECT_DROP_THRESHOLD = 3; // Pixels leeway for a perfect drop
...
export default function StakeBuilderGamePage() {
  ...
  const fetchUserHearts = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/games/hearts?userId=${currentUser.id}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      if (data.success && data.hearts && data.hearts[GAME_TYPE_IDENTIFIER]) {
        ...
        const heartsFromServer = typeof gameHeartData === 'number' ? gameHeartData : (gameHeartData.count || MAX_POOLED_HEARTS);
        const nextRegenFromServer = typeof gameHeartData === 'number' ? null : (gameHeartData.nextRegen || null);

        setPooledHearts(Math.min(heartsFromServer, MAX_POOLED_HEARTS));
        if (nextRegenFromServer && heartsFromServer < MAX_POOLED_HEARTS) {
          setNextHeartRegenTime(new Date(nextRegenFromServer).getTime());
        } else if (heartsFromServer >= MAX_POOLED_HEARTS) {
          setNextHeartRegenTime(null);
        } else if (heartsFromServer < MAX_POOLED_HEARTS && !nextRegenFromServer) {
          // If server doesn't provide nextRegen, but hearts are missing, try to set one based on local logic or default
           setNextHeartRegenTime(Date.now() + HEART_REGEN_DURATION_MS);
        }
        setGameState(heartsFromServer > 0 ? 'idle' : 'waiting_for_hearts');
      } else {
        ...
        const initialRegenTime = savedRegenTime !== null ? parseInt(savedRegenTime, 10) : null;
        if (initialHearts < MAX_POOLED_HEARTS && initialRegenTime === null) {
            setNextHeartRegenTime(Date.now() + HEART_REGEN_DURATION_MS);
        } else {
            setNextHeartRegenTime(initialRegenTime);
        }
        toast({ title: 'Heart Sync Using Local Data', description: data.error || 'Could not sync hearts with server, using local data.', variant: 'default' });
  ...
  }, [currentUser?.id, toast]); // Removed pooledHearts from deps as it might cause loops

  useEffect(() => {
    if (currentUser?.id && !isFetchingUser && gameState === 'loading_user') { // Only fetch if user loaded and we are in initial loading state
        fetchUserHearts();
    }
  ...
  useEffect(() => {
    ...
    const checkBackendReplenish = async () => {
      if (!currentUser?.id) return; // Guard against currentUser becoming null
      try {
        const res = await fetch('/api/games/replenish-hearts', { 
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: currentUser.id, gameType: GAME_TYPE_IDENTIFIER }), // Ensure gameType is passed if your API expects it
        });
        const data = await res.json();
        if (data.success && data.hearts && data.hearts[GAME_TYPE_IDENTIFIER]) {
          const gameHeartData = data.hearts[GAME_TYPE_IDENTIFIER];
          const heartsFromServer = typeof gameHeartData === 'number' ? gameHeartData : (gameHeartData.count || 0);
          const nextRegenFromServer = typeof gameHeartData === 'number' ? null : (gameHeartData.nextRegen || null);
          ...
          console.warn('Periodic heart replenish check failed:', data.error || data.message);
        }
      } catch (error) {
    ...
    checkBackendReplenish(); // Initial check on mount
  ...
  const spawnNewBlock = useCallback((currentTopWidth: number, visualCurrentTopY: number, speed: number) => {
    const newBlockWidth = Math.max(currentTopWidth * 0.95, MIN_BLOCK_WIDTH * 1.5); // Ensure it doesn't shrink too fast
    setCurrentBlock({
      x: Math.random() < 0.5 ? 0 - newBlockWidth/3 : gameAreaWidth - newBlockWidth*2/3, // Start slightly on screen
  ...
  const initializeNewGameAttempt = useCallback(() => {
    ...
    spawnNewBlock(baseBlock.width, baseBlock.y - 0, BLOCK_SLIDE_SPEED_START); // stackVisualOffsetY is 0 here
  ...
  const startGameAttempt = useCallback(async () => {
    ...
    if (pooledHearts <= 0 && gameState !== 'ad_viewing') { // Prevent starting if already viewing ad for heart
    ...
    try {
      ...
      if (!data.success) {
        ...
        // Attempt to re-sync hearts from backend if API call failed
        await fetchUserHearts(); 
        setIsGameApiLoading(false); // Ensure loading state is reset
        return; // Stop game start
      }
      // If API call was successful, update local state
      setPooledHearts(prevHearts => {
        const newHearts = Math.max(0, prevHearts - 1); // Ensure hearts don't go negative
        if (newHearts < MAX_POOLED_HEARTS && nextHeartRegenTime === null) { // Start timer only if it wasn't already running
          setNextHeartRegenTime(Date.now() + HEART_REGEN_DURATION_MS);
        }
        return newHearts;
      });
  ...
  const processAttemptOver = useCallback(async () => {
    let diamondsEarned = currentAttemptDiamonds; // Diamonds from consecutive perfects are already tracked
    // Add diamonds from total gold score if any
    if (currentAttemptGold > 0) {
        let diamondsFromGoldScore = currentAttemptGold * 0.0001; // Example: 1 diamond per 10000 gold
        if (diamondsFromGoldScore > 0 && diamondsFromGoldScore < 0.001) { // Miniscule amount, maybe round up or ignore
            diamondsFromGoldScore = 0.001;
        }
        diamondsEarned += diamondsFromGoldScore;
    }
    diamondsEarned = parseFloat(diamondsEarned.toFixed(4)); // Ensure precision
    ...
    if (currentUser?.id) {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.totalDiamonds !== undefined) setMockUserDiamondBalance(data.totalDiamonds); // Update local mock balance
        } else {
      ...
      // Offline mode or user not loaded
  ...
  }, [currentUser, currentAttemptGold, currentAttemptDiamonds, stackedBlocks.length, toast]); // Added currentAttemptDiamonds
  ...
  const continueCurrentAttempt = useCallback(() => { 
    if (stackedBlocks.length > 0) {
        ...
        // This case should ideally not be reached if game over is handled properly
        initializeNewGameAttempt();
    }
  ...
  const handleDropBlock = useCallback(() => {
    ...
    // Safeguard: Ensure block has moved onto the screen
    const blockHalfWidth = currentBlock.width / 2;
    if ((currentBlock.direction === 1 && currentBlock.x < -blockHalfWidth) || // Still significantly off-screen left
        (currentBlock.direction === -1 && currentBlock.x + blockHalfWidth > gameAreaWidth + blockHalfWidth )) { // Still significantly off-screen right
      return; // Block not sufficiently on screen, ignore drop
    }

    setGameState('dropping'); // Visual state, actual logic runs below
    ...
    if (overlapWidth > MIN_BLOCK_WIDTH / 2) {
      ...
      if (Math.abs(currentBlock.x - visualTopStackBlockX) < PERFECT_DROP_THRESHOLD && 
          Math.abs(currentBlock.width - topStackBlock.width) < PERFECT_DROP_THRESHOLD + 2) { // Slightly more lenient on width for perfect
        isPerfectDrop = true;
        ...
        if (newConsecutivePerfects >= 3) {
          ...
          setConsecutivePerfectDrops(0); 
          toast({ description: <span className="flex items-center"><Gem className="h-4 w-4 mr-1 text-sky-400"/> Triple Perfect! +{DIAMONDS_FOR_THREE_CONSECUTIVE_PERFECT_DROPS.toFixed(4)} Diamonds</span>, duration: 2000, className: "bg-primary/20 border-primary/50" });
        }
      } else {
        gainedGoldThisDrop = GOLD_FOR_SUCCESSFUL_NON_PERFECT_DROP;
        setConsecutivePerfectDrops(0); // Reset on non-perfect drop
      ...
      const newBlockY = topStackBlock.y - INITIAL_BLOCK_HEIGHT; // Relative to stack origin
      ...
      if (visualNewBlockTopY < GAME_AREA_HEIGHT / 2.5 && stackedBlocks.length + 1 > 5) { // Start scrolling after 5 blocks & if it reaches midpoint
        setStackVisualOffsetY(prevOffset => prevOffset + INITIAL_BLOCK_HEIGHT);
      }
      
      const nextSpeed = BLOCK_SLIDE_SPEED_START + ((stackedBlocks.length +1) * BLOCK_SLIDE_SPEED_INCREMENT); // Speed increases with stack height
      spawnNewBlock(newBlockWidth, newBlockY - stackVisualOffsetY, nextSpeed); // Pass visual top Y for spawning
      setGameState('playing');
    } else {
      // Missed the block
      processAttemptOver();
    }
  }, [gameState, currentBlock, stackedBlocks, consecutivePerfectDrops, spawnNewBlock, processAttemptOver, toast, gameAreaWidth, stackVisualOffsetY, initializeNewGameAttempt]);
  ...
  }, [gameState, currentBlock, gameAreaWidth]); // currentBlock should be here to re-bind if it changes

  useEffect(() => {
    if (gameState === 'playing' && currentBlock) {
      if (!gameLoopRef.current) { // Start loop only if not already running
          gameLoopRef.current = requestAnimationFrame(gameLoop);
      }
    } else {
  ...
  }, [gameState, gameLoop, currentBlock]); // Dependencies that control the loop
    ...
    if (gameState === 'ad_viewing' && isAdDialogOpen && adTimer > 0) {
      ...
      const processAdReward = async () => {
        if (!currentUser?.id) {
            ...
            // Determine previous sensible state
            setGameState(gameState === 'gameover_attempt' && adPurpose === 'revive_attempt' ? 'gameover_attempt' : (pooledHearts > 0 ? 'idle' : 'waiting_for_hearts'));
            return;
        }
        ...
        try {
            if (adPurpose === 'revive_attempt') {
                // TODO: Consider an API call here to log ad revive usage if needed by backend
                ...
                const res = await fetch('/api/games/reward-ad-heart', { // Placeholder API
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ userId: currentUser.id, gameType: GAME_TYPE_IDENTIFIER }),
                });
                ...
                if (data.success && data.hearts && data.hearts[GAME_TYPE_IDENTIFIER]) {
                    const gameHeartData = data.hearts[GAME_TYPE_IDENTIFIER];
                    const heartsFromServer = typeof gameHeartData === 'number' ? gameHeartData : (gameHeartData.count || pooledHearts + 1);
                    const nextRegenFromServer = typeof gameHeartData === 'number' ? null : (gameHeartData.nextRegen || null);
                    ...
                    if (nextRegenFromServer && heartsFromServer < MAX_POOLED_HEARTS) {
                        ...
                        setNextHeartRegenTime(null);
                    } else if (heartsFromServer < MAX_POOLED_HEARTS && !nextRegenFromServer && nextHeartRegenTime === null) {
                        setNextHeartRegenTime(Date.now() + HEART_REGEN_DURATION_MS); // Start timer if it wasn't running
                    }
                    ...
                    // Fallback to local update if API fails or is placeholder
                    setPooledHearts(prev => {
                        const newHearts = Math.min(prev + 1, MAX_POOLED_HEARTS);
                    ...
                    toast({ title: "Ad Reward (Local)", description: data.error || "Could not grant heart from ad server, applied locally.", variant: data.error ? "destructive" : "default" });
                    setGameState(pooledHearts +1 > 0 ? 'idle' : 'waiting_for_hearts');
                }
            }
             // Determine next logical state after ad view if not reviving
            if (adPurpose !== 'revive_attempt') {
            ...
            setAdTimer(AD_REVIVE_DURATION_S); // Reset timer for next use
  ...
  }, [gameState, isAdDialogOpen, adTimer, adPurpose, currentUser?.id, continueCurrentAttempt, pooledHearts, nextHeartRegenTime, toast, fetchUserHearts]);


  const closeAdDialogEarly = useCallback(() => {
    setIsAdDialogOpen(false);
    // Restore previous sensible game state
    if (adPurpose === 'revive_attempt') {
        setGameState('gameover_attempt'); 
    } else { // gain_pooled_heart or null
        setGameState(pooledHearts > 0 ? 'idle' : 'waiting_for_hearts');
    }
  ...
  }, [pooledHearts, toast, adPurpose]); // Ensure adPurpose is a dependency
  ...
  const handleSpendDiamondsToContinue = useCallback(async () => {
    ...
    // const cost = DIAMONDS_FOR_THREE_CONSECUTIVE_PERFECT_DROPS; // This was an earning, not a cost. Define revive cost if needed.
    const costToContinue = 0.01; // Example cost
    
    // TODO: Call backend to deduct diamonds and confirm continuation
    // For now, client-side simulation
    if (mockUserDiamondBalance >= costToContinue) {
        const newSimulatedBalance = parseFloat((mockUserDiamondBalance - costToContinue).toFixed(4));
        setMockUserDiamondBalance(newSimulatedBalance); 
        // Potentially update backend here
        
        toast({
            description: (
                <span className="flex items-center">
                    <Gem className="h-4 w-4 mr-1 text-sky-400" /> -{costToContinue.toFixed(4)} Diamonds spent. Attempt continued! (Simulated)
                </span>
            ),
        });
        continueCurrentAttempt();
    } else {
        toast({ title: "Not Enough Diamonds", description: `You need ${costToContinue.toFixed(4)} diamonds to continue. Your balance: ${mockUserDiamondBalance.toFixed(4)} (Simulated)`, variant: "destructive"});
    }
  }, [currentUser?.id, mockUserDiamondBalance, continueCurrentAttempt, toast]);
  ...
  const canContinueWithDiamonds = mockUserDiamondBalance >= 0.01; // Example cost
  ...
  return (
    <AppShell>
      <div className="flex flex-col items-center justify-center w-full min-h-[calc(100vh-var(--header-height)-var(--bottom-nav-height,0px))] md:min-h-[calc(100vh-var(--header-height))] pt-2 pb-2 md:pt-4 md:pb-4 bg-gradient-to-br from-slate-900 via-purple-900/40 to-slate-900 text-slate-100 overflow-hidden">
        <div
            className="flex flex-col items-center w-full max-w-md mx-auto p-2 sm:p-4 bg-slate-800/80 text-slate-100 rounded-xl shadow-2xl border-2 border-primary/30 relative overflow-hidden backdrop-blur-sm"
        ...
        >
          <div className="flex justify-between items-center w-full mb-3 px-3 py-3 bg-slate-900/60 rounded-t-lg shadow-md">
            <div className="text-xl font-bold flex items-center">
                {getDisplayedScore()}
            </div>
            <div className="flex items-center space-x-1.5">
              {Array.from({ length: MAX_POOLED_HEARTS }).map((_, i) => (
                <Heart key={`life-${i}`} className={cn("h-7 w-7 transition-all duration-300", i < pooledHearts ? "text-red-500 fill-red-500 animate-[pulse-glow_1.5s_infinite_ease-in-out]" : "text-slate-600")} />
              ))}
            </div>
          </div>

          {(gameState === 'idle' || gameState === 'waiting_for_hearts') && pooledHearts < MAX_POOLED_HEARTS && timeToNextHeart && (
            <div className="w-full text-center py-2 px-3 bg-slate-900/70 rounded-md mb-3 text-sm shadow">
          ...
          <div
            ref={gameAreaRef} className="relative bg-black/60 border-2 border-slate-700/80 rounded-md overflow-hidden shadow-inner"
            style={{ 
                height: `${GAME_AREA_HEIGHT}px`, 
                width: `${gameAreaWidth}px`,
                backgroundImage: 'linear-gradient(180deg, hsl(var(--primary)/0.15) 0%, hsl(var(--accent)/0.05) 40%, hsl(var(--background)/0.7) 100%)',
              ...
              {stackedBlocks.map(block => (
                <div key={block.id}
                  className={cn("absolute rounded-sm",
                    block.isPerfect && "ring-2 ring-yellow-300 ring-offset-1 ring-offset-black/50" // Static ring for perfect
                  )}
                  style={{ 
                      left: `${block.x}px`, 
                      top: `${block.y}px`, 
            ...
            {currentBlock && (gameState === 'playing' || gameState === 'dropping') && (
              <div className="absolute rounded-sm border border-white/20"
                style={{ 
                    ...
                    willChange: 'left, top, width', // Hint for current block
                }}
              />
            )}
                ...
                {gameState === 'waiting_for_hearts' && (
                    <>
                        ...
                        <p className="text-lg mb-3 text-slate-200">
                            {timeToNextHeart ? `Next heart in: ${timeToNextHeart}` : (pooledHearts < MAX_POOLED_HEARTS ? "Calculating..." : "Hearts are full!")}
                        </p>
                        {canWatchAdForPooledHeart && (
                    ...
                    <div className="space-y-2.5 w-full max-w-xs">
                      ...
                      {canContinueWithDiamonds && (
                         <Button onClick={handleSpendDiamondsToContinue} disabled={isGameApiLoading} variant="outline" size="lg" className="w-full border-sky-400 text-sky-400 hover:bg-sky-400 hover:text-slate-900 transition-colors">
                            ...
                            Use {0.01.toFixed(3)} <Gem className="inline h-3 w-3"/> to Continue
                         </Button>
                      )}
