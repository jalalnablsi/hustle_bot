...
export function SkyHighStackerGame() {
  ...
  const fetchGameData = useCallback(async (userId: string) => {
    setIsApiLoading(true);
    try {
      const res = await fetch(`/api/games/high-scores?userId=${userId}&gameType=${GAME_TYPE_IDENTIFIER}`);
      if (res.ok) {
        const data = await res.json();
        if (data.success) setHighScore(data.highScore || 0);
      }
    } catch (error) {
      toast({ title: 'Error Loading Game Data', variant: 'destructive' });
    } finally {
  ...
  }, [currentUser, stackedBlocks.length, currentAttemptGold, currentAttemptDiamonds, updateUserSession, toast, GAME_TYPE_IDENTIFIER]);
  ...
  const spawnNewBlock = useCallback((currentTopWidth: number, visualCurrentTopY: number) => {
    ...
    setCurrentBlock({
      x: Math.random() < 0.5 ? 0 : gameAreaWidth - currentTopWidth,
  ...
  const initializeNewAttempt = useCallback(() => {
    if (gameAreaWidth <= 0) {
        toast({ title: "Game Area Error", description: "Could not initialize game area. Please resize your window or try again.", variant: "destructive" });
        setGameState('idle');
        return;
    }
    ...
    setContinuesUsed(0);
    setStackOffsetY(0);
    const baseBlock = { id: 'base', x: (gameAreaWidth - INITIAL_BASE_WIDTH) / 2, y: GAME_AREA_HEIGHT_MIN - INITIAL_BLOCK_HEIGHT, width: INITIAL_BASE_WIDTH, color: 'hsl(var(--muted))' };
  ...
  const handleDropBlock = useCallback(() => {
    ...
    if (isPerfect) {
      ...
      if (newConsecutive >= 3) {
        ...
        setConsecutivePerfects(0);
        toast({ description: <span className="flex items-center"><Gem className="h-4 w-4 mr-1 text-sky-400" /> 3x Perfect! +{gainedDiamonds}💎</span>, duration: 1500 });
      }
    } else {
    ...
    if (newBlock.y - stackOffsetY < GAME_AREA_HEIGHT_MIN / 2.5) {
      setStackOffsetY(o => o + INITIAL_BLOCK_HEIGHT);
    }
    spawnNewBlock(newBlockWidth, newBlockY - stackOffsetY);
    setGameState('playing');
  }, [gameState, currentBlock, stackedBlocks, consecutivePerfects, processGameOver, spawnNewBlock, stackOffsetY, toast]);
  ...
  return (
    <div ref={gameAreaRef} className="flex flex-col flex-grow items-center justify-between w-full bg-gradient-to-br from-slate-900 via-purple-950/80 to-slate-900 text-slate-100 p-2 overflow-hidden" style={{ minHeight: 'calc(100vh - 128px)'}}>
